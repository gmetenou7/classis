...
const schema = yup.object({
  name: yup.string().required(formValidation.REQUIRED_FIELD),
  email: yup
    .string()
  ...
  password: yup
    .string()
    .min(8, formValidation.PASSWORD_TOO_SHORT)
    .required(formValidation.PASSWORD_ABSENT),

  rePassword: yup
    .string()
    .oneOf([yup.ref("password")], formValidation.REPASSWORD_NOT_MATCH)
    .required(formValidation.REPASSWORD_ABSENT),
});

const SignUp: React.FC = () => {
  const { handleSubmitFn, mutation, errors, register } = useAuthForm({
    schema,
    mutationKey: ["sign-up", 1],
    apiRoute: "/api/sign-up",
  });
  return (
    <>
      <Head>
        <title>Inscription | Classis</title>
        <meta name="description" content="This is Sign-up page for Classis" />
      </Head>
      <span className="mb-1.5 block font-medium">
        Start for freeDébutez gratuitement
      </span>
      <h2 className="mb-9 text-2xl font-bold text-black dark:text-white sm:text-title-xl2">
        Inscrivez vous à Classis
      </h2>
      ...
      <form onSubmit={handleSubmitFn}>
        ...
        <div className="mb-6">
          ...
          {errors.rePassword && (
            <p className="text-rose-500">{errors.rePassword.message}</p>
          )}
        </div>
